...
На прошлой лекции было показано, что задача о максимальном потоке решается за хоть сколько-то приемлимое время с помощью алгоритма Эдмондса-Карпа. 
...
Всюду на лекции мы считаем, что задана некоторая сеть $\mathcal{N} = (G, s, t, c)$.
...
Из леммы 8.4.1 следует, что $f + f'$ тоже есть поток и более того $|f + f'| > |f|$, что противоречит определению $f$. В обратную сторону существует контрпример:
...
\end{equation*}
$E_L$ получается же только лишь рёбрами, которые идут из $V_{d}$ в $V_{d + 1}$ для всякого $d$.
...
Будем обозначать $\texttt{FIND\_BLOCK\_FLOW(}\mathcal{N}\texttt{)}$ функцию, которая по сети $\mathcal{N}$ находит в ней блокирующий поток.
...
\begin{figure*}[h]
    ...
    \begin{tikzpicture}[
        ...
        vertex/.style={circle, draw, thick, minimum size=9mm, inner sep=1pt, 
                    font=\bfseries\sffamily, fill=white},
...
\St Слоистая сеть содержит все кратчайшие пути от $s$ до $t$ и всякий путь в ней от $s$ до $t$ кратчайший.
\begin{proof}
...
\Lm После каждой итерации схемы Диница расстояние от истока к стоку в остаточной сети строго возрастает.
\begin{proof}

    \textit{...Здесь типа будет доказательство с лекции...}
...
\begin{proof}
    ...
    Очевидно, что расстояние не может уменьшаться. Действительно, используя лемму 9.2.2 (если убрать из неё алгоритм Эдмондса-Карпа и работать в предположениях, что мы просто пускаем потоки вдоль кратчайших путей) получаем требуемое.
    ...
    Предположим, что он был в $G_0$. В силу того, что данный путь был кратчайшим от $s$ до $t$ в $G_0$, то, по утверждению 10.4.1, он находился в слоистой сети $G^L_0$.
...
\begin{proof}
    ...
    По лемме 10.5.2, расстояние от $s$ до $t$ строго возрастает. В тоже время максимальная длина пути от $s$ до $t$ не более $|V| - 1$. Получили требуемое.
...
Если емкости вершин целочисленны (а в рамках наших лекций это так), то на самом деле можно дать другую верхнюю оценку на количество итераций схемы Диницы.
...
\Def Пусть $\Ne$ -- сеть и $v \in V$. Тогда $c^+(v) \overset{def}{=} \min\limits_{(u, v) \in E} c(u, v)$.
...
\begin{proof}
    ...
    Рассмотрим вершину $v \in V - s, t$.

    \begin{multline*}
        c_f^+(v) = \sum\limits_{(u, v) \in E} c_f(u, v) = \sum\limits_{(u, v) \in E} c(u, v) - f(u, v) = \\ 
            = \sum\limits_{(u, v) \in E} c(u, v) - \sum\limits_{(u, v) \in E} f(u, v) = 
    \end{multline*}
...
\begin{proof}
    ...
    В силу того, что после каждой итерации схемы расстояние от истока до стока уменьшается строго на 1, то в сверху количество итераций оценивается как $\delta_\Ne (s, t)$.
