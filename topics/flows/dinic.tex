...
\Def \textit{Циркуляцией} в сети $\Ne$ будем называть такую функцию $f: V \times V \to \Z$, которая удовлетворяет следующим аксимомам:
\begin{enumerate}
    \item $f(e) \leq c(e), \forall e \in E$
    \item $\forall u, v \in V \hookrightarrow f(u, v) = -f(v, u)$
    \item $\forall u \in V \hookrightarrow \sum\limits_{v \in V} f(u, v) = 0$
\end{enumerate}

\Note Как можно видеть по определению цикруляции -- это по сути поток только без истока и стока.

\Lm[regular-flow-pos-edge] Если $f$ -- регулярный ненулевой поток, то существует ребро с положительным потоком, выходящим из $s$.
\begin{proof}
    
    $f$ -- регулярный ненулевой поток $\Rightarrow |f| > 0$.

    Предположим противное, что такого ребра не существует. Значит,
    \begin{equation*}
        \forall v \in V - s \hookrightarrow f(s, v) \leq 0
    \end{equation*}
    Тогда:
    \begin{equation*}
        |f| = \sum\limits_{v \in V - s} f(s, v) \leq 0
    \end{equation*}
    Получили противоречие.

\end{proof}

\Lm[regular-flow-minus-path] Пусть $f$ -- регулярный ненулевой поток и $f'$ -- некоторый поток, который проходит по ребрам, по которым проходит $f$, представляет собой простой путь из $s$ в $t$ и $|f'| \leq |f|$. Тогда $f - f'$ -- регулярный поток
\begin{proof}
    
    Пусть $f'$ представляет собой некоторый простой путь $e_1, e_2, e_3, \dots e_m$. Пусть $e_1 = (s, v_0)$. То что $f - f'$ представляет собой поток совершенно очевидно. Заметим, что $f'$ допустимый поток в остаточной сети $\Ne_{f - f'}$. А значит, по \LmRef[лемме]{lm-flow-sum}:
    \begin{equation*}
        |f - f'| + |f'| = |f - f' + f'| = |f|
    \end{equation*}
    \begin{equation*}
...
\Lm[regular-flow-minus-circ] Пусть $f$ -- регулярный ненулевой поток и $f'$ -- произвольная циркуляция, которая проходит по ребрам, по которым проходит $f$. Тогда $f - f'$ -- регулярный поток.
\begin{proof}
    В силу того, что всякий цикл не влияет на величину потока, то убирание этого цикла, поток не меняет.
\end{proof}

\Th{О декомпозиции} Пусть $\Ne$ -- сеть и $f$ -- \textit{регулярный} поток в этой сети. Тогда найдутся простые пути из $s$ в $t$ $P_1, P_2, \dots, P_n$, а также циклы $C_1, C_2, \dots C_m$.
...
\begin{proof}
Считаем, что $f \not\equiv 0$, иначе разложение тривиально.

Пусть из $s$ выходит хотя бы одно ребро $(s, v)$ с положительным потоком. Если такового нет, то мы приходим к противоречию с  \LmRef[леммой]{regular-flow-pos-edge}. Иначе перейдем по этому ребру в вершину $v$. 
...
Будем это делать до тех пор, пока $|f| > 0$ (а значит поток остается регулярным по \LmRef[лемме]{regular-flow-minus-path} и \LmRef[лемме]{regular-flow-minus-circ}). Далее, в следствие нашего алгоритма могли еще остаться циклы с $f(e) > 0$. Например, в такой ситуации:
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        node style/.style={
...
Если опустить свойство регулярности потока, то теорема, вообще говоря, неверна. Возьмем сеть вида:
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[node style] (t) at (3, 0) {$t$};

        \draw[edge style, blue] (t) -- node[above, font=\large] {$1/1$} (s);

    \end{tikzpicture}

\end{figure}

Как видим, $|f| = -1$, а также, очевидно, нельзя такой поток разложить на простые пути из $s$ в $t$ и циклы.

\Cons Пусть $\Ne$ -- сеть и $f$ -- \textit{регулярный} поток в этой сети. Тогда найдутся простые пути из $s$ в $t$ $P_1, P_2, \dots, P_n$, а также циклы $C_1, C_2, \dots C_m$.
\begin{equation*}
